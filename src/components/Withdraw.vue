  ...
  <div class="window white add-liquidity">
        ...
        <li v-for='(currency, i) in Object.keys(currencies)'>
          <label :for="'currency_'+i" class='currency_label'>
            ...
            <span v-show='withdrawc'>{{ currencies[currency] }}
	                    	<span
                            v-show="!(currency == 'usdt' && currentPool == 'usdt') && currentPool != 'susdv2'">(in {{ currency | capitalize }})</span>
                    	</span>
          </label>
          <input type="text"
                 :id="'currency_'+i"
    ...
    <fieldset v-show="currentPool != 'susd'">
      ...
      <ul>
        <li v-show="!['susdv2','tbtc','ren'].includes(currentPool)">
          <input type='radio' id='to_cur_comb' name="to_cur" :value='10' :checked='to_currency === 10'
        ...
        <li v-for='(currency, i) in Object.keys(currencies)' class='withdrawin'>
          <input type="radio" :id="'to_cur_'+i" name="to_cur" :value='i' :checked='to_currency === i'
                 @click='handleCheck(i)'>
          <label :for="'to_cur_'+i">
            <img
                :class="{'token-icon': true, [currency+'-icon']: true, 'y': withdrawc, [currentPool]: true}"
    ...
    <div id='max_slippage' v-show='showWithdrawSlippage'><span>Max slippage:</span>
      ...
      <label for="custom_slippage" @click='customSlippageDisabled = false'>
        <input type="text" id="custom_slippage_input" :disabled='customSlippageDisabled' name="custom_slippage_input"
    ...
    <div id='withdraw_buttons'>
      ...
      <router-link v-show="currentPool == 'susdv2' && oldBalance > 0" class='button' to='/susd/withdraw'
                   id='withdrawold'>Withdraw old
      </router-link>
      <button @click='migrateUSDT' v-show="currentPool == 'usdt'">Migrate to PAX</button>
      <button id="remove-liquidity" @click='handle_remove_liquidity' v-show="currentPool == 'susd'">Withdraw old
      </button>
      {{ currentPool }}
    ...
    async unstake(amount, exit = false) {
      ...
      try {
        await new Promise((resolve, reject) => {
          currentContract.swap.methods.withdraw(amount.toFixed(0, 1))
              .send({
